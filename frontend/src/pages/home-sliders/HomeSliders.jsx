...
import { homeSliders } from '../../api/admin';
...
const HomeSliders = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['home-sliders', page],
    queryFn: async () => {
      const response = await homeSliders.getAll({ page, limit });
      return response.data.data || [];
    },
  });

  const createMutation = useMutation({
    mutationFn: homeSliders.create,
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => homeSliders.update(id, data),
    onSuccess: () => {
  ...
  const handleSubmit = (e) => {
    ...
    uploadData.append('buttonText', formData.get('buttonText') || '');
    uploadData.append('buttonLink', formData.get('buttonLink') || '');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">إدارة سلايدر الصفحة الرئيسية</h2>
          <p className="text-sm text-gray-500 mt-1">إدارة البانرات والسلايدر في الصفحة الرئيسية</p>
        </div>
        <button
          onClick={() => {
            setEditingItem(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-card p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">إجمالي السلايدر</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{slidersList.length}</p>
            </div>
        ...
        <div className="glass-card p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">السلايدر النشط</p>
              <p className="text-2xl font-bold text-green-600 mt-1">
                {slidersList.filter(s => s.isActive).length}
              </p>
            </div>
      ...
      <div className="glass-card">
        <DataTable
          ...
          isLoading={isLoading}
          searchable={true}
          searchPlaceholder="بحث في السلايدر..."
        />
      </div>
      ...
      <Modal
        ...
        title={editingItem ? 'تعديل السلايدر' : 'إضافة سلايدر جديد'}
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="label">صورة السلايدر</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="label">رابط الزر</label>
              <input
                name="buttonLink"
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                ...
                />
                <label className="font-semibold text-gray-900">تفعيل السلايدر</label>
              </div>
            </div>
          </div>
