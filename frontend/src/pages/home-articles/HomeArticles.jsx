...
import { homeArticles } from '../../api/admin';
...
const HomeArticles = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await homeArticles.getAll({ page, limit });
      return response.data.data || [];
    },
  });

  const createMutation = useMutation({
    mutationFn: homeArticles.create,
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => homeArticles.update(id, data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: homeArticles.delete,
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">إدارة مقالات الصفحة الرئيسية</h2>
          <p className="text-sm text-gray-500 mt-1">إدارة المقالات والخدمات المقالية المعروضة في الصفحة الرئيسية</p>
        </div>
        <button
          onClick={() => {
            setEditingItem(null);
      ...
      <Modal
        ...
        title={editingItem ? 'تعديل المقال' : 'إضافة مقال جديد'}
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="label">رابط المقال</label>
            <input
              name="link"
              type="text"
              defaultValue={editingItem?.link}
              className="input"
              placeholder="مثال: /articles/communication"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                ...
                />
                <label className="font-semibold text-gray-900">تفعيل المقال</label>
              </div>
            </div>
          </div>
