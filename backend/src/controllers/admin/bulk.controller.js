 ...
 * Bulk Moderate Posts
 */
export const bulkModeratePosts = async (req, res, next) => {
  try {
    ...
    await prisma.activityLog.create({
      data: {
        ...
        entityType: 'POST',
        description: `Bulk ${action} ${result.count} posts. Reason: ${reason || 'No reason provided'}`,
        ipAddress: req.ip,
        userAgent: req.get('user-agent')
      }
    });
    ...
    res.json({
      success: true,
      data: { count: result.count },
      message: `${result.count} posts ${action} successfully`
    });
  } catch (error) {
    logger.error('Bulk moderate posts error:', error);
    next(error);
  }
};
  ...
  try {
    ...
    await prisma.activityLog.create({
      data: {
        ...
        action: 'BULK_DELETE',
        entityType: 'COMMENT',
        description: `Bulk deleted ${result.count} comments`,
        ipAddress: req.ip,
        userAgent: req.get('user-agent')
      }
    });
    ...
    res.json({
      success: true,
      data: { count: result.count },
      message: `${result.count} comments deleted successfully`
    });
  } catch (error) {
    logger.error('Bulk delete comments error:', error);
    next(error);
  }
};
